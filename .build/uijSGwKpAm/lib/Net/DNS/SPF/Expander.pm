...
sub new_spf_records {
    ...
    for my $domain ( keys %$lengths ) {
        ...
        if ( $lengths->{$domain} > $self->maximum_record_length ) {
            say "Length of domain is longer";
...
sub _new_records_from_arrayref {
    ...
    say "Length text data: ", length($txtdata);
    say "Text data: $txtdata";
...
sub _new_records_from_partition {
    ...
    my $number_of_partitions = (int( $record_length / $max_length ) + 1)
